...
def create_appbar(page, settings, on_new_game):
    ...
    page.appbar = ft.AppBar(
        leading=ft.Image(src=f"/images/poker.png"),
        leading_width=30,
        title=ft.Text(
            "Solitaire By Jithuze",
            weight=ft.FontWeight.BOLD,
            ),
        bgcolor=ft.colors.BLACK,
        actions=[
            ft.FilledButton(text="New Game",on_click=new_game_clicked,),
    ...
    rules_dialog = ft.AlertDialog(
        title=ft.Text("Game Rules"), content=rules_md, on_dismiss=lambda e: print("Dialog dismissed!")
    )